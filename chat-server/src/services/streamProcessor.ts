...
export class StreamProcessor {
    ...
    async processStream(
        ...
        logger.info('StreamProcessor finished', {
            contentLength: result.accumulatedContent.length,
            toolCallsCount: result.completeToolCalls.length,
            completeToolCalls: JSON.stringify(result.completeToolCalls, null, 2),
        });
    ...
    private handleToolCallsDelta(toolCalls: any[]): void {
        logger.info('Received tool calls delta', {
