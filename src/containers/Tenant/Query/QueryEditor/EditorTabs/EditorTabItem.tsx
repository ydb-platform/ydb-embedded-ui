...
import type {DropdownMenuItem, DropdownMenuItemAction} from '@gravity-ui/uikit';
...
export function EditorTabItem({
    ...
    const createMenuAction = React.useCallback(
        (action: () => void): DropdownMenuItemAction<unknown> => {
            return (event) => {
                event.stopPropagation();
                action();
            };
