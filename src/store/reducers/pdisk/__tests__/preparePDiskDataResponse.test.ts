...
describe('preparePDiskDataResponse', () => {
    ...
    test('Should set Recipient on donor VDisk and synchronize donor ref StringifiedId', () => {
        // Donor VDisk at slot 1002 — a top-level whiteboard entry
        const donorVDisk = {
            VDiskId: {
                GroupID: 2181038080,
                GroupGeneration: 1,
        ...
        // The donor should have a Recipient reference pointing back to the acceptor
        expect(donorSlot?.SlotData.Recipient).toBeDefined();
        expect(donorSlot?.SlotData.Recipient?.NodeId).toEqual(1);

        // Find the acceptor slot
        const acceptorSlot = vDiskSlots?.find(
            (slot) => slot.SlotType === 'vDisk' && slot.SlotData.VDiskSlotId === 1001,
        );
        expect(acceptorSlot).toBeDefined();

        // The acceptor's Donors entry should have its StringifiedId synchronized
        // with the real donor's StringifiedId (VDiskID-based, not slot-based)
