...
export async function setupPopupScrollMocks(page: Page, nodeCount = 1, disksPerNode = 1) {
    ...
    await page.route(/\/viewer\/|\/meta\//, async (route) => {
        ...
        if (url.pathname.endsWith('/nodes')) {
            ...
            json = {Name: 'popup test', Overall: EFlag.Green, Nodes: nodeCount};
